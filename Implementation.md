# Telegram AI Bot on Convex — Implementation Guide
...
└──────────────┬──────────────────┘
               ...
               ▼
┌─────────────────────────────────┐
│  Convex Action: processMessage  │
│  - Query recent conversation    │
│  - Build prompt with context    │
│  - Call Claude / OpenAI API     │
│  - Store assistant response     │
│  - POST sendMessage to Telegram │
└─────────────────────────────────┘
```
...
│   ├── telegram.ts          # Core bot logic (actions)
│   ├── messages.ts          # Message storage (mutations + queries)
│   └── lib/
│       ├── ai.ts            # AI provider abstraction
│       └── telegramApi.ts   # Telegram Bot API helpers
├── package.json
├── tsconfig.json
...
// ── Main message processor (scheduled by the webhook) ───────────
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    // Return immediately — Telegram is happy
    return new Response("OK", { status: 200 });
  }),
});
...
2. `/setprivacy` → Select your bot → **Disable** (so the bot sees group messages for context)
3. `/setdescription` → Set a description
4. `/setcommands` → Set:
   ```
...
- **Token counting**: Estimate tokens (1 token ≈ 4 chars) and cap at ~4,000 tokens of context
...
Telegram may rarely send the same update twice. You can deduplicate by storing `update_id` and checking before processing. For most bots this isn't necessary — the worst case is a duplicate AI response.
...
| Mutation (store msg) | 2 (user + assistant)       | ↑ counted together   | ↑            |
| Query (get context)  | 1-2                        | ↑                    | ↑            |
| Action compute       | ~5s × 0.5GB ≈ 0.0007 GB-hr | 20 GB-hr/mo          | $0.33/GB-hr  |
...
Telegram sends photos as file IDs. You can download them via the Bot API, convert to base64, and send to Claude's vision API — all within a Convex action.
